...
// Deterministic 1024-dim one-hot embedding (a distinct, non-zero unit vector per
// chunk). Keeps RAG retrieval tests hermetic — no Voyage call — while giving the
// pgvector cosine branch valid vectors to rank. Cosine on a zero vector is NaN,
// so one-hot (never all-zero) matters.
...
// One KnowledgeChunk per lesson per locale so the assistant's search_course_content
// tool has a corpus. With VOYAGE_API_KEY unset in tests, retrieval degrades to the
// keyword branch — these rows back that path.
async function seedKnowledgeChunks(): Promise<number> {
  let idx = 0;
  for (const l of LESSONS) {
    const sourceId = `${l.course}/${l.moduleId}/${l.slug}`;
    for (const loc of [
      { locale: "EN", title: l.titleEN, content: l.bodyEN },
      { locale: "ZH", title: l.titleZH, content: l.bodyZH },
    ] as const) {
      await prisma.$executeRaw`
        INSERT INTO "KnowledgeChunk"
          (id, source, "sourceId", "moduleId", "certLevel", locale, title, content, "chunkIndex", embedding, "updatedAt")
        VALUES
          (${`kc-${idx}`}, ${"LESSON"}, ${sourceId}, ${l.moduleId}, ${l.certLevel},
           ${loc.locale}, ${loc.title}, ${loc.content}, ${0}, ${oneHot(idx)}::vector, NOW())`;
      idx++;
    }
  }
  return idx;
...
async function main() {
  ...
  const chunks = await seedKnowledgeChunks();
  console.log(
    `✓ test fixtures — questions: ${basicQ} basic / ${advancedQ} advanced, lessons: ${lessons}, chunks: ${chunks}`,
  );
}
