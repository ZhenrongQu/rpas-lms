...
## 19. Implementation Delta Record (as of 2026-07-04)
...
> **⚠️ 2026-07-04 correction — the "verification is trustworthy" claims below are SUPERSEDED.** Docker isolation and the hidden holdout protect the *host*, but do NOT prove a fix is correct. The code under test and the verifier share one process/security domain inside the container, so the code can forge `result.json`, call `process.exit(0)`, read the injected holdout source with raw `fs`, or special-case the visible test. Treat all Docker/Vitest evidence as **heuristic**, not proof. `PROPOSED` authorization is being re-anchored to an external black-box attestation the code under test cannot forge; until a real attestor exists, a `production-black-box` run must terminate `NEEDS_HUMAN`, never `PROPOSED`. See `docs/superpowers/specs/2026-07-04-production-black-box-verification-design.md`. Affected items below carry inline corrections.
...
- **Fail-closed check results** — `CheckResult` is a discriminated union (`CompletedCheck | InfraFailure`); infrastructure failures can never be misread as red or green, and Vitest runs must produce a parseable report (no exit-code-only greens). *(Correction 2026-07-04: a parseable report proves the JSON is report-SHAPED, not that it came from a trusted execution — code under test can write a forged `result.json`. Structural validation is a sanity check, not proof.)*
- **Repairer trust boundary + isolation guard** — repairer trust is an unforgeable module-private `WeakSet` brand granted only by the `FixtureRepairer` constructor (no production mutator, no test backdoor, subclassing blocked). `assertIsolatedForUntrusted`, enforced inside `runFixAttempt`, refuses any untrusted repairer (notably `LlmRepairer`) unless both `runCheck` and `runHoldout` are Docker-isolated (`--network=none`, `--cap-drop=ALL`, `no-new-privileges`, `--read-only`, non-root `--user 1000:1000`, resource limits, write access only to a disposable report mount). *(Correction 2026-07-04: this isolation protects the HOST from untrusted code; it does NOT prove the fix is correct — the verifier still runs in the same domain as the code under test. See the section banner and the black-box spec.)*
- **Hidden holdout** — a verification test injected only after patch capture, so it is not in the diff; a fix that games the visible check fails the holdout and routes to `NEEDS_HUMAN`. *(Correction 2026-07-04: "never visible to the repair author" was wrong — the holdout is written into `src/` and the code under test runs in the same Vitest process, so it can read the holdout source with raw `fs` and special-case it. Not-in-the-diff ≠ invisible; only a guest-external verdict closes this.)*
...
- The graded script-fixture LLM eval (`pnpm eval:repair`) is permanently disabled — exit-code-only checks cannot prove execution, so the untrusted-author eval moved to `pnpm real-repair-eval` (real Vitest + Docker). *(Correction 2026-07-04: "report-proof" was wrong — Docker+Vitest is heuristic, not proof. Under the black-box decision `real-repair-eval` is a `production-black-box` run and, until a real attestor exists, its terminal state must be `NEEDS_HUMAN: black-box-verification-unavailable`, never `PROPOSED`. It may still score LLM repair quality.)*
