...
export default async function LearnHub({ params }: Props) {
  ...
  const cards = await Promise.all(
    COURSES.map(async ({ course, access }) => ({
      course,
      access,
      modules: (await getCourseModules(locale as RouteLocale, course)).length,
      lessons: await getCourseLessonCount(course),
    })),
  );
  ...
  return (
    <div className="module-landing">
      ...
      <div className="course-grid">
        {cards.map(({ course, access, modules, lessons }) => (
